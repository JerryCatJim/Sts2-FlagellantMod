...
public static class ShalebajiNodePatch
{
	...
	public static void Postfix(NCombatUi __instance)
	{
		...
		if (old == null)
		{
			_cachedShalebajiNode = GD.Load<PackedScene>("res://Flagellant/Scenes/ShalebajiNode.tscn").Instantiate<Control>();
...
public static class ShalebajiNodeActivatePatch
{
	public static void Postfix(CombatState state)
	{
		...
		Log.Info(">>>[FlagellantMod]Character.Id is " + player.Character.Id.ToString());
		if (player.Character.Id.ToString() == "CHARACTER.FLAGELLANT-FLAGELLANT")
		{
